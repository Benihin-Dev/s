    ...
    <div className="fixed top-0 left-0 right-0 shadow-md md:shadow-none  opacity-90 sm:opacity-100 rounded-2xl sm:bg-transparent z-10 block md:flex w-11/12  sm:w-4/5 mx-auto mt-5 justify-between items-center">
      <div className="flex mr-2 items-center justify-between">
        <img
          ...
          className="navBarImg h-9 border  w-8  bg-white items-start  rounded-full  flex justify-center overflow-hidden"
        />
        ...
        <div className="block md:hidden">
          {value === false ? (
            <svg
              ...
              strokeWidth={1.5}
              stroke="currentColor"
              className="w-8 h-8 cursor-pointer hover:text-white text-gray-400"
            >
              <path
                strokeLinecap="round"
            ...
            <svg
              ...
              strokeWidth={1.5}
              stroke="currentColor"
              className="w-8 h-8 cursor-pointer hover:text-gary-400  text-white"
              onClick={() => {
                handlemenu();
              }}
            >
      ...
      <ul
        className={`block whiteDarkli ${
          value
            ? "hidden md:flex w-2/3 gap-20 items-center py-2"
            : "block md:flex md:w-2/3  "
        }transparentwhite shadow-lg  px-5  relative md:gap-12 text-center md:w-2/3 rounded-3xl md:bg-opacity-80 md:items-end md:justify-around py-2`}
      >
        <li
          className={
            activeSection === "about-section" ? "border-b-2 border-gray-50" : ""
          }
        >
          <Link
            to="about-section"
