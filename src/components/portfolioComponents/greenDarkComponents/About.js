    ...
    <div className="block sm:flex w-4/5 mt-12 mx-auto md:mt-5 overflow-hidden">
      ...
      <div className="w-full aboutUser upFromDown mt-1 mt sm:mt-12 md:mt-32  lg:mt-48 xl:mt-64 sm:pl-6 md:w-7/12 ">
        ...
        <h1 className="text-4xl md:text-5xl text-white">
          I'm{" "}
          <span className="name text-green-400 tracking-normal">
            {aboutData.name}{" "}
          </span>
          <p className=" text-lg sm:text-2xl p-0 text-white tracking-widest">
            {" "}
            <ReactTyped
          ...
          <p className=" text-xs md:text-sm text-justify text-slate-400">
            {aboutData.description}
          </p>
        </h1>
        <div className="mt-2 sm:mt-1  flex justify-end items-center">
          <a href={aboutData.cv}>
            <button className=" border rounded-2xl border-green-400 text-slate-200 hover:text-black hover:bg-green-400 text-xs sm:text-normal py-1 px-3">
              Download CV
            </button>
          </a>
        </div>
      ...
      <img
        src={aboutData.profile}
        className="hidden sm:flex h-60 visibleAnimation profileImg userProfileImg mt-8 md:mt-28 md:w-4/12 justify-center overflow-hidden"
      />
    </div>
  );
}
