    ...
    <div className="block sm:flex w-4/5 mt-12 mx-auto md:mt-5 overflow-hidden">
      ...
      <div className="w-full aboutUser upFromDown mt-1 mt sm:mt-12 md:mt-32  lg:mt-48 xl:mt-64 sm:pl-6 md:w-7/12 ">
        ...
        <h1 className="text-4xl md:text-5xl text-slate-50">
          I'm <span className="name text-[#eeee38] tracking-normal">{aboutData.name} </span>
          ...
          <p className=" text-justify text-xs md:text-sm text-yellow-200">
            {aboutData.description}
          </p>
        </h1>
        <div className="mt-2 sm:mt-1  flex justify-end items-center">
          <a href={aboutData.cv}>
            <button className=" border rounded-2xl border-[#eeee38] hover:bg-[#eeee38]  text-slate-50 hover:text-black text-xs sm:text-normal py-1 px-3">
              Download CV
            </button>
          </a>
        </div>
      ...
      <img
        src={aboutData.profile}
        className="hidden sm:flex  visibleAnimation profileImg userProfileImg mt-8 md:mt-28 md:w-4/12 justify-center"
      />
    </div>
  );
}
